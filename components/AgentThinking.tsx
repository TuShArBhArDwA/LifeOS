...
import { Eye, Cpu, CheckSquare, Calendar, Target, Bell, Loader2 } from 'lucide-react';
...
/* ─── Phase timeline ───────────────────────────────────────────────────────
   ...
   2 → all parallel agents start (active)     2 000 ms
   3 → Task Agent done, rest still active     1 800 ms
   4 → Schedule Agent done, rest active       1 600 ms
   5 → Placement Agent done, rest active      1 400 ms
   6 → Reminder Agent done → all complete
─────────────────────────────────────────────────────────────────────────── */
const DURATIONS = [2500, 2000, 2000, 1800, 1600, 1400];
...
/* ─── Main component ─────────────────────────────────────────────────────── */
type AgentThinkingProps = { intent?: string };
...
export default function AgentThinking({ intent }: AgentThinkingProps) {
  ...
  const ALL_IDS = ['intake', 'orchestrator', 'task', 'schedule', 'placement', 'reminder'];
  ...
  // Overall progress: how many phases are done / total
  const progressPct = Math.round((phase / TOTAL_PHASES) * 100);

  // How many parallel agents are done
    ...
    <div className="w-full max-w-xl mx-auto py-12 px-6">
      ...
      <div className="flex flex-col items-center mb-10">
        ...
        <p className="text-sm text-white/35 mt-1">
          Processing across parallel pipelines
        </p>
      </div>
        ...
        {!allDone ? (
          <>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-semibold text-brand-400 tabular-nums">
                {progressPct}%
              </span>
            </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-center gap-3 mt-2 px-4 py-3.5 rounded-2xl border border-brand-500/15 bg-brand-500/5">
              ...
              <div className="flex-1 overflow-hidden">
                <p
                  key={msgIndex}
                  className="text-sm font-medium text-white/70 animate-fade-slide-in"
                >
          ...
          <div
            ...
            className={`flex items-center gap-4 px-5 py-4 rounded-2xl border transition-all duration-700 ${
              step.status === 'active'
                ...
                : step.status === 'done'
                ? 'bg-surface-elevated/20 border-surface-border opacity-45'
      ...
      <div className="grid grid-cols-2 gap-3">
        {parallel.map((step, i) => {
          ...
          return (
            <div
              key={step.id}
              className={`flex flex-col gap-3 p-4 rounded-2xl border transition-all duration-700 ${
            ...
            >
              ...
              {/* Label */}
              <p
                className={`text-xs font-medium leading-snug transition-all duration-500 ${
              ...
              {/* Per-card progress bar (visible only when active or done) */}
              {(isActive || isDone) && (
                <div className="h-0.5 w-full bg-surface-border rounded-full overflow-hidden">
                  <div
                    ...
                    style={
                      isActive
                        ? {
                            // Animate width from 5% → ~85% while active
                            width: '85%',
                            transition: `width ${DURATIONS[2 + i] ?? 1800}ms ease-out`,
                          }
                        : undefined
                    }
                  />
      ...
      {/* ── Completion summary ── */}
      {phase >= 2 && (
        <p className="text-center text-xs text-white/30 mt-6 font-medium tabular-nums">
